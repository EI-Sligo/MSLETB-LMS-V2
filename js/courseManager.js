...
export const courseManager = {
    ...
    launchContent: async (id, type, url) => {
        ...
        // ROBUST YOUTUBE DETECTION (Standard, Share, Embed, Shorts)
        const isYouTube = url && /(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/.test(url);
        ...
        if(type === 'simulator') {
            ...
            // FIX: If it is YouTube and NOT forced external, open in Viewer Popup
    ...
    openViewer: (url, type, canDownload) => { 
        ...
        if(titleEl) titleEl.innerText = "Viewing Content";
        ...
        if (ytMatch && ytMatch[1]) {
            ...
            if(titleEl) titleEl.innerText = "YouTube Video";
            return;
        } 
        ...
        if (type === 'video' || ['mp4', 'webm'].includes(ext)) { 
            ...
            body.innerHTML = `<iframe src="https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(url)}" class="w-full h-full border-0 bg-white"></iframe>`;
        } else { 
   ...
   moveItem: async (table, id, direction) => {
    ...
    bulkCreate: async (type, parentId) => { const title = prompt(`Enter ${type} title:`); if(!title) return; const payload = { title, is_visible: true }; if(type === 'section') payload.course_id = state.activeCourse.id; else if(type === 'module') payload.section_id = parentId; else if(type === 'unit') { payload.module_id = parentId; payload.total_hours_required = 0; } await sb.from(type + 's').insert([payload]); const modal = document.querySelector('.fixed.z-\\[70\\]'); if(modal) { modal.remove(); courseManager.openBulkEdit(); } else { courseManager.loadSyllabus(); } },
